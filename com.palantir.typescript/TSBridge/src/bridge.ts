    ...
    export class Service implements TypeScriptServiceBridge.IService {
        ...
        private addFileWithReferences(request: IAddFileWithReferencesRequest): TypeScriptServiceBridge.IResult {
            ...
            for (var i=0; i<numOfReferences; i++) {
                ...
                if (referencedFile.length > 0 && referencedFile[0] === "/") { // absolute path
                    localRoot = "";
                } else { // local path
                    localRoot = root;
                }
        ...
        private removeFile(request: IRemoveFileRequest): TypeScriptServiceBridge.IResult {
            ...
            var result = {"resultType": request.command, "resultValid": rawResult};
            if (result.resultValid === false) {
                var errorMessage = "File cannot be removed (probably because it already has been removed or was never added)";
                var eResult: TypeScriptServiceBridge.IError = {"resultType": request.command, "resultValid": rawResult, "errorMessage": errorMessage};
                return eResult;
            }
            return result;
        }
        ...
        private invalidCommand(request: TypeScriptServiceBridge.IRequest): TypeScriptServiceBridge.IResult {
            return TypeScriptServiceBridge.TSServiceBridge.invalidResult("It turns out the command: \"" + request.command + "\" is a Bad Auto Complete Request");
        }
    }
}
...
module TypeScriptServiceBridge {
    ...
    export class TSServiceBridge {
        ...
        private services: IService[] = []; // For every service you want to call, you have to provide a server for it here.
        ...
        private processRequest(request) { // hands off the request to the appropriate IService
